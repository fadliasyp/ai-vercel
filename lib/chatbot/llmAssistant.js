...
export function resolveLlmAssistantConfig(env = process.env) {
  ...
  return {
    ...
    enabled:
      mode !== "legacy" &&
      (naturalizer.enabled || geminiFallbackEnabled),
    naturalizer,
...
function shouldUseGeminiComposerFallback(status = null) {
...
export async function runLlmAnswerComposer({
  ...
  if (!config.preferGemini && config.naturalizer?.enabled) {
  ...
  const useGemini =
    ...
    (!config.naturalizer?.enabled ||
      config.preferGemini ||
      shouldUseGeminiComposerFallback(composerStatus));
