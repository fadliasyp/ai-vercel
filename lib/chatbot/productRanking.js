...
export function explainBestRuleBased(best, candidates = [], rawQuestion = "") {
  ...
  const catalogNotes = extractProductComparisonNotes(best);
  if (catalogNotes.strengths.length) {
    reasons.push(
      `kelebihan dari deskripsi katalog: **${catalogNotes.strengths.slice(0, 3).join("; ")}**`,
    );
  }
  if (catalogNotes.caveats.length) {
    reasons.push(
      `pertimbangan dari deskripsi katalog: **${catalogNotes.caveats.slice(0, 3).join("; ")}**`,
    );
  }
    ...
    altBlock =
      ...
      alts
        .map((p, i) => {
          ...
          const noteText = [
            notes.strengths[0] ? `kelebihan: ${notes.strengths[0]}` : "",
            notes.caveats[0] ? `pertimbangan: ${notes.caveats[0]}` : "",
          ]
            .filter(Boolean)
            .join("; ");

          return `• ${i + 1}. **${p.name}** - ${formatRupiah(p.numericPrice)} (${stockText}${salesText})${noteText ? `; ${noteText}` : ""}`;
        })
        .join("\n");
  }
