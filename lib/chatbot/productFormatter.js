...
const PRODUCT_NOTE_HEADING_PATTERN =
  /^(?:detail\s*(?:&|dan)?\s*)?(?:kelebihan|keunggulan|kekurangan|catatan(?:\s+yang)?\s+perlu\s+diperhatikan|pros?|cons?)(?:\s*[/:&-]\s*(?:kelebihan|keunggulan|kekurangan|catatan(?:\s+yang)?\s+perlu\s+diperhatikan|pros?|cons?))*\s*:?$/i;
...
export function extractProductComparisonNotes(product = {}) {
  ...
  for (const statement of statements) {
    if (PRODUCT_NOTE_HEADING_PATTERN.test(statement)) continue;
