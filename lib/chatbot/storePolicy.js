...
const BULK_PURCHASE_OFFER_PATTERN =
  /\b(?:beli|ambil|borong|belanja|checkout)\b.{0,90}\b(?:\d+\s*(?:barang|produk|item|pcs)|total|juta|ribu)\b.{0,90}\b(?:diskon|potongan|harga\s+paket|gratis\s+ongkir)\b|\b(?:diskon|potongan|harga\s+paket|gratis\s+ongkir)\b.{0,90}\b(?:beli|ambil|borong|belanja|checkout|total)\b/i;
const BUY_ONE_GET_ONE_PATTERN =
  /\b(?:buy|beli|ambil)\s*(?:barang|produk|item)?\s*(?:1|satu|one)\b.{0,50}\b(?:gratis|free|get|dapat)\s*(?:1|satu|one)\b|\b(?:bogo|buy\s*one\s*get\s*one|buy\s*1\s*get\s*1)\b|\bgratis\s*(?:1|satu)\b.{0,50}\b(?:kalau\s+|jika\s+)?(?:beli|ambil)\s*(?:1|satu)\b/i;
...
export function looksLikeBulkPurchaseOfferQuestion(question = "") {
  return BULK_PURCHASE_OFFER_PATTERN.test(
    normalizeIndonesianCommerceText(question),
  );
}

export function looksLikeBuyOneGetOneQuestion(question = "") {
  return BUY_ONE_GET_ONE_PATTERN.test(
    normalizeIndonesianCommerceText(question),
  );
}

export function extractBulkPurchaseOfferContext(question = "") {
  const text = normalizeIndonesianCommerceText(question);
  const quantityMatch = text.match(
    /\b(\d+)\s*(?:barang|produk|item|pcs)\b/i,
  );
  const totalMatch = text.match(
    /\btotal(?:nya)?(?:\s+(?:jadi|sekitar))?\s*(?:rp\.?\s*)?(\d+(?:[.,]\d+)?)\s*(juta|jt|ribu|rb)?\b/i,
  );
  const destinationMatch = String(question || "").match(
    /\bgratis\s+ongkir\s+ke\s+([a-z][a-z\s-]{1,40}?)(?=\s+(?:nggak|ngga|enggak|engga|gak|ga|ya|sih|dong)\b|[?.!,]|$)/i,
  );
  const amount = totalMatch
    ? Number(totalMatch[1].replace(",", "."))
    : 0;
  const unit = String(totalMatch?.[2] || "").toLowerCase();
  ...
  return {
    ...
    cartTotal: amount > 0 ? amount * multiplier : null,
    location: String(destinationMatch?.[1] || "").trim(),
    asksFreeShipping: /\bgratis\s+ongkir\b/i.test(text),
  };
}
...
export function buildBulkPurchaseOfferMessage({
  quantity = null,
  cartTotal = null,
  location = "",
  asksFreeShipping = false,
} = {}) {
  const purchase = [
    Number(quantity) > 0 ? `${Number(quantity)} barang` : "",
    Number(cartTotal) > 0
      ? `total sekitar Rp ${Number(cartTotal).toLocaleString("id-ID")}`
      : "",
  ]
    .filter(Boolean)
    .join(" dengan ");
  const destination = String(location || "").trim();
  const offer = asksFreeShipping
    ? `potongan tambahan atau gratis ongkir${destination ? ` ke ${destination}` : ""}`
    : "potongan tambahan";

  return (
    `${purchase ? `Untuk rencana pembelian ${purchase}, ` : ""}${offer} belum bisa dijanjikan otomatis. ` +
    "Kelayakannya bergantung pada produk yang dibeli, promo yang sedang aktif, nilai transaksi, dan persetujuan admin. " +
    "Harga katalog tetap menjadi acuan sampai penawaran dikonfirmasi.\n\n" +
    "Admin Robot Jadul dapat memeriksa paket pembelianmu dan memberi keputusan yang pasti tanpa membuatmu mengulang pertanyaan."
  );
}
