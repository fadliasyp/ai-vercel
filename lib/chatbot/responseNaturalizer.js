...
export function buildSuggestionGenerationContext({
  ...
  const recentQuestions = uniqueStrings(recentActions).slice(-12);
  ...
  const eligibleKeys = (SUGGESTION_KEYS_BY_INTENT[intent] || [])
    .filter((key) => {
      ...
      if (key === "product_price" && products.every((item) => item.has_price)) {
        return false;
      }
      if (key === "product_stock" && products.every((item) => item.has_stock)) {
        return false;
      }
      return true;
    });
  const freshKeys = eligibleKeys.filter((key) => !recent.has(key));
  const allowedActions = (freshKeys.length >= 3 ? freshKeys : eligibleKeys)
    .map((key) => ({
      key,
        ...
        messages: [
          {
            ...
            content:
              ...
              "Jika suggestion_generation berisi allowed_actions, buat maksimal tiga pertanyaan lanjutan baru yang paling mungkin dibutuhkan user setelah jawaban ini. Setiap item wajib memakai action_key yang diizinkan, product_indexes yang tersedia, dan kalimat question yang natural. question harus berupa kalimat yang akan diucapkan pelanggan kepada chatbot, bukan tawaran dari asisten; jangan mulai dengan 'Mau aku bantu', 'Aku bisa bantu', atau bentuk serupa. Jangan menanyakan harga atau stok produk jika available_products menandai fakta tersebut sudah tampil. Prioritaskan kondisi, kelengkapan, spesifikasi, kecocokan, alternatif, atau perbandingan yang relevan. Hindari pertanyaan dalam recent_questions. Jangan mengulang permintaan user, jangan mengarang produk atau kemampuan, dan jangan menulis URL/data sensitif. " +
...
export async function naturalizeResponseWithGroq(
  ...
  for (let index = 0; index < models.length; index += 1) {
    try {
      ...
      const rankedActions = uniqueStrings([
        ...generatedActions,
        ...rankedFallback,
      ]).slice(0, 3);
