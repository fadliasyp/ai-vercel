...
export function buildSuggestionGenerationContext({
  ...
  const products = (Array.isArray(payload.products) ? payload.products : [])
    .map((product) => String(product?.name || "").trim().slice(0, 180))
    .filter(Boolean)
    .slice(0, 5)
    .map((name, index) => ({ index, name }));
  const completed = completedSuggestionKeys(userQuestion);
  const allowedActions = (SUGGESTION_KEYS_BY_INTENT[intent] || [])
    .filter((key) => {
      const spec = SUGGESTION_ACTIONS[key];
      return spec && spec.products <= products.length && !completed.has(key);
    })
    .map((key) => ({
      key,
