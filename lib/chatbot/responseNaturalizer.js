...
// Leaves room for JSON overhead within the 850 completion-token limit.
const MAX_NATURALIZER_TEXT_CHARS = 2400;
...
function immutablePlaceholder(index) {
  let value = index;
  let suffix = "";
  do {
  ...
  return `{{RJLOCK${suffix}}}`;
}

function immutableSpans(payload = {}, text = "") {
  const source = String(text || "");
  const spans = [];
  const addMatches = (pattern, trimEnd = false) => {
  ...
  for (const match of source.matchAll(/\*\*([^*\n]+)\*\*/g)) {
    ...
    spans.push({
      start: match.index + 2,
      end: match.index + 2 + value.length,
      value,
    });
  }
  addMatches(/https?:\/\/[^\s)>\]]+/gi, true);
  addMatches(/\d+(?:[.,]\d+)*(?:\s*%|\s*(?:pcs|kg|cm))?/gi);

  const lower = source.toLowerCase();
  for (const name of mentionedProductNames(payload, source)) {
    const needle = name.toLowerCase();
    let start = lower.indexOf(needle);
    while (start >= 0) {
      spans.push({ start, end: start + name.length, value: source.slice(start, start + name.length) });
      start = lower.indexOf(needle, start + name.length);
    }
  }
...
function lockImmutableTextFields(payload = {}) {
  let placeholderIndex = 0;
  const lockedPayload = { ...payload };
  const locks = {};

  for (const field of TEXT_FIELDS) {
    const source = String(payload[field] || "");
    const fieldLocks = immutableSpans(payload, source).map((span) => ({
      ...span,
      placeholder: immutablePlaceholder(placeholderIndex++),
    }));
    let lockedText = source;
    for (const lock of [...fieldLocks].reverse()) {
      lockedText =
        lockedText.slice(0, lock.start) +
        lock.placeholder +
        lockedText.slice(lock.end);
    }
    lockedPayload[field] = lockedText;
    locks[field] = fieldLocks;
  }

  return { payload: lockedPayload, locks };
}
...
function restoreImmutableTextFields(original = {}, candidate = {}, locks = {}) {
  const restored = { ...candidate };
  const repairedFields = [];
  const placeholderPattern = /\{\{RJLOCK[A-Z]+\}\}/g;

  for (const field of TEXT_FIELDS) {
    const fieldLocks = Array.isArray(locks[field]) ? locks[field] : [];
    const text = typeof candidate[field] === "string" ? candidate[field] : "";
    const found = text.match(placeholderPattern) || [];
    const placeholdersIntact =
      found.length === fieldLocks.length &&
      fieldLocks.every((lock) => occurrenceCount(text, lock.placeholder) === 1) &&
      found.every((placeholder) =>
        fieldLocks.some((lock) => lock.placeholder === placeholder),
      );
    const exactFactsIntact =
      found.length === 0 &&
      fieldLocks.every(
        (lock) =>
          occurrenceCount(text, lock.value) ===
          fieldLocks.filter((item) => item.value === lock.value).length,
      );

    if (!placeholdersIntact && !exactFactsIntact) {
      restored[field] = String(original[field] || "");
      repairedFields.push(field);
      continue;
    }

    restored[field] = fieldLocks.reduce(
      (value, lock) => value.replace(lock.placeholder, lock.value),
      text,
    );
  }

  return { candidate: restored, repairedFields };
}
...
function preserveTextFieldShape(original = {}, candidate = {}) {
  return {
    ...candidate,
    ...Object.fromEntries(
      TEXT_FIELDS.map((field) => {
        ...
        return [field, before ? after || before : ""];
      }),
    ),
  };
}

function textFieldPayload(payload = {}, field = "") {
  return {
    ...payload,
    ...Object.fromEntries(
      TEXT_FIELDS.map((name) => [name, name === field ? payload[name] || "" : ""]),
    ),
  };
}

function naturalizerFieldContracts(payload = {}, locks = {}) {
  return Object.fromEntries(
    TEXT_FIELDS.map((field) => {
      ...
      return [
        field,
        {
          must_remain_empty: !text.trim(),
          numbers: protectedNumbers(text),
          urls: protectedUrls(text),
          phrases: protectedMarkdownPhrases(text),
          product_names: mentionedProductNames(payload, text),
          immutable_placeholders: (locks[field] || []).map(
...
export function naturalizedResponseSafetyIssue(original = {}, candidate = {}) {
  for (const field of TEXT_FIELDS) {
    const before = String(original[field] || "").trim();
    const after =
      typeof candidate[field] === "string" ? candidate[field].trim() : "";

    if (!before && after) return `unexpected_${field}`;
    if (before && !after) return `missing_${field}`;
    if (after.length > Math.max(before.length * 2, before.length + 220)) {
      return `expanded_${field}`;
    }
  }
  ...
  if (
    ...
    return !sameValues(protectedNumbers(beforeText), protectedNumbers(afterText))
      ? "protected_numbers_changed"
      : "protected_urls_changed";
  }
  ...
  if (
    ...
    return "protected_phrase_removed";
  }
  return "";
}

export function isSafeNaturalizedResponse(original = {}, candidate = {}) {
  return !naturalizedResponseSafetyIssue(original, candidate);
}

function repairUnsafeTextFields(original = {}, candidate = {}) {
  const repaired = { ...candidate };
  const repairedFields = [];

  for (const field of TEXT_FIELDS) {
    const before = textFieldPayload(original, field);
    const after = textFieldPayload(candidate, field);
    if (!naturalizedResponseSafetyIssue(before, after)) continue;
    repaired[field] = String(original[field] || "").trim();
    repairedFields.push(field);
  }

  return { candidate: repaired, repairedFields };
}
  ...
  try {
        ...
        messages: [
          {
            ...
            content:
              ...
              "editable_text dapat memuat token {{RJLOCK...}} yang mewakili fakta tervalidasi. Salin setiap token tepat satu kali, persis, dan tetap di field asal; jangan menghapus, menggandakan, memindahkan, menerjemahkan, atau mengubah token tersebut. " +
              "Patuhi field_contracts: setiap token numbers, urls, phrases, product_names, dan immutable_placeholders harus tetap persis di field yang sama; field must_remain_empty tidak boleh diisi. " +
          ...
          },
          {
            ...
            content: JSON.stringify({
              ...
              editable_text: naturalizerInput(editablePayload),
              field_contracts: naturalizerFieldContracts(
                editablePayload,
                immutableLocks,
              ),
              verified_facts:
    ...
    if (!response.ok) {
      ...
      if (
        ...
        error.reason = "failed_generation";
      }
      throw error;
    }
...
export async function naturalizeResponseWithGroq(
  ...
  const editableText = combinedText(payload).trim();
  if (!editableText) {
    reportStatus(onStatus, {
      provider: "template",
      naturalized: false,
  ...
  if (editableText.length > MAX_NATURALIZER_TEXT_CHARS) {
    reportStatus(onStatus, {
      provider: "template",
      naturalized: false,
  ...
  const immutable = lockImmutableTextFields(payload);

  for (let index = 0; index < models.length; index += 1) {
    try {
      const rawCandidate = await requestNaturalizedJson({
        payload,
        editablePayload: immutable.payload,
        immutableLocks: immutable.locks,
        userQuestion,
        intent,
        conversationContext,
        actionCandidates,
      ...
      const restored = restoreImmutableTextFields(
        payload,
        rawCandidate,
        immutable.locks,
      );
      const shapedCandidate = preserveTextFieldShape(
        payload,
        restored.candidate,
      );
      const firstValidationReason = restored.repairedFields.length
        ? "immutable_placeholder_changed"
        : naturalizedResponseSafetyIssue(payload, shapedCandidate);
      const repaired = repairUnsafeTextFields(payload, shapedCandidate);
      const candidate = repaired.candidate;
      const repairedFields = uniqueStrings([
        ...restored.repairedFields,
      ...
      const validationReason = naturalizedResponseSafetyIssue(
        payload,
        candidate,
      );
      if (validationReason) {
      ...
      const textChanged = TEXT_FIELDS.some(
        (field) =>
          String(payload[field] || "").trim() !==
          String(candidate[field] || "").trim(),
      );
      if (!textChanged && firstValidationReason) {
        reportStatus(onStatus, {
          provider: "groq",
          model: models[index],
          naturalized: false,
          reason: "unsafe_rewrite_no_safe_change",
          validation_reason: firstValidationReason,
      ...
      reportStatus(onStatus, {
        ...
        model: models[index],
        naturalized: true,
        reason: repairedFields.length
          ? "success_with_field_fallback"
          : "success",
        repaired_fields: repairedFields,
        suggestions_generated: generatedActions.length > 0,
      });
