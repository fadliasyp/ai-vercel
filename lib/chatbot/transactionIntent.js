...
export function looksLikeHowToBuyQuestion(q = "") {
  const s = String(q || "").toLowerCase();
  return (
    /\b(?:langkah|tahapan|alur|proses)\s+(?:untuk\s+)?(?:beli|membeli|order|pesan|checkout|pembelian)\b|\bcheckout\b.{0,50}\b(?:gimana|bagaimana|caranya|seperti\s+apa)\b/.test(
      s,
    ) ||
...
export function looksLikeShippingOriginQuestion(q = "") {
  ...
  const asksWarehouseLocation =
    /\b(?:gudang|warehouse)(?:nya)?\s*(?:ada\s+)?(?:di\s*mana|dimana|mana|lokasi(?:nya)?|berlokasi)\b/.test(
      s,
    ) ||
    /\b(?:di\s*mana|dimana|lokasi)\b.*\b(?:gudang|warehouse)\b/.test(s) ||
    /\b(?:alamat|lokasi)(?:\s+lengkap)?\b.*\b(?:gudang|warehouse)\b/.test(s);
...
const INTERNATIONAL_DESTINATIONS = [
  ...
  ["Singapura", /\b(?:singapura|singapore)\b/i],
  ["Brunei", /\bbrunei\b/i],
  ["Thailand", /\bthailand\b/i],
  ["Filipina", /\b(?:filipina|philippines)\b/i],
  ["Vietnam", /\bvietnam\b/i],
  ["Jepang", /\b(?:jepang|japan)\b/i],
  ["Korea", /\b(?:korea|korsel|south\s+korea)\b/i],
  ["Tiongkok", /\b(?:tiongkok|china)\b/i],
  ["Hong Kong", /\bhong\s*kong\b/i],
  ["Taiwan", /\btaiwan\b/i],
  ["Australia", /\baustralia\b/i],
  ["Amerika Serikat", /\b(?:amerika(?:\s+serikat)?|usa|united\s+states)\b/i],
  ["Inggris", /\b(?:inggris|united\s+kingdom|uk)\b/i],
  ["Kanada", /\b(?:kanada|canada)\b/i],
  ["Selandia Baru", /\b(?:selandia\s+baru|new\s+zealand)\b/i],
  ["Eropa", /\b(?:eropa|europe)\b/i],
];

export function extractInternationalShippingDestination(q = "") {
  const text = String(q || "");
  const hasShippingContext =
    /\b(?:ongkir|ongkos\s+kirim|biaya\s+kirim|kirim|dikirim|pengiriman|shipping|kurir|packing)\b/i.test(
      text,
    );
  if (!hasShippingContext) return "";

  const matched = INTERNATIONAL_DESTINATIONS.find(([, pattern]) =>
    pattern.test(text),
  );
  if (matched) return matched[0];
  if (/\b(?:luar\s+negeri|internasional|international|overseas)\b/i.test(text)) {
    return "luar negeri";
  }
  return "";
}

export function looksLikeInternationalShippingQuestion(q = "") {
  return Boolean(extractInternationalShippingDestination(q));
}

export function buildInternationalShippingMessage(destination = "luar negeri") {
  return (
    `Pengiriman ke **${destination}** perlu dikonfirmasi langsung dengan Admin Robot Jadul. ` +
    "Cek ongkir domestik di chatbot belum mencakup pengiriman internasional, jadi aku tidak akan menebak tarif atau total akhirnya.\n\n" +
    "Admin akan membantu memastikan kurir internasional yang tersedia, berat volumetrik, packing tambahan, asuransi, ongkir, serta kemungkinan bea masuk. " +
    "Total dalam USD juga perlu mengikuti kurs dan biaya layanan saat transaksi dikonfirmasi."
  );
}
...
export function buildTransactionPolicyMessage(
  ...
  if (looksLikeBulkPurchaseOfferQuestion(question)) {
    sections.push(
      buildBulkPurchaseOfferMessage(extractBulkPurchaseOfferContext(question)),
    );
  }
