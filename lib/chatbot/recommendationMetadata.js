...
const JUNK_PATTERN = /\b(?:junk|rongsok|part\s+only)\b/i;
const NEGATED_JUNK_PATTERN =
  /\b(?:bukan|tidak\s+(?:termasuk|tergolong))\b[^.!?;\n]{0,80}\b(?:junk|rongsok|part\s+only)\b/gi;
...
export function deriveRecommendationMetadata(product = {}) {
  ...
  const junk = JUNK_PATTERN.test(text.replace(NEGATED_JUNK_PATTERN, " "));
