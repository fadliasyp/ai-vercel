...
const STORE_BACKGROUND_PATTERN =
  /\b(?:asal[\s-]*usul|awal\s+mula|sejarah|berdiri\s+sejak|kapan\s+(?:berdiri|didirikan)|siapa\s+(?:pendiri|pemilik)|didirikan\s+oleh|cerita\s+(?:tentang|soal))\b/i;
const STORE_REFERENCE_PATTERN =
  /\b(?:robot\s+jadul|toko\s+ini|tokonya|toko\s+robot\s+jadul)\b/i;
...
export function looksLikeStoreBackgroundQuestion(question = "") {
  const text = normalizeIndonesianCommerceText(question);
  return (
    STORE_REFERENCE_PATTERN.test(text) && STORE_BACKGROUND_PATTERN.test(text)
  );
}
