...
function clarificationForProduct(products = [], facets = []) {
  const command = facets.includes("product_condition")
    ? "Jelaskan kondisi"
    : facets.includes("stock")
      ? "Cek stok"
      : facets.includes("promo") || facets.includes("budget")
        ? "Cek harga dan promo"
        : "Tampilkan detail";
...
function inferPrimaryIntent(text, facets, purposes) {
  ...
  if (facets.includes("promo") || facets.includes("budget")) {
    return "price_promo";
  }
...
export function analyzeCompoundQuestion(question = "", context = {}) {
  ...
  if (
    ...
    VAGUE_PRODUCT_REFERENCE.test(text) &&
