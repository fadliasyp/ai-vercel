...
export function preserveExplicitTopicSwitch(question = "", route = {}) {
  const explicitlySwitchesTopic =
    /\b(?:abaikan|lupakan|tinggalkan)\b[\s\S]{0,160}\b(?:sekarang|kali\s+ini|beralih|ganti)\b/i.test(
      String(question || ""),
    );
  return explicitlySwitchesTopic
    ? { ...route, topic_relation: "topic_switch" }
    : route;
}
...
- Permintaan mencari atau memilih produk sesuai kebutuhan/tujuan memakai
  recommendation, termasuk "produk mana yang paling cocok", "bagusnya",
  "worth it", hadiah, pajangan, pemula, atau kolektor baru.
- Menilai satu produk spesifik yang sudah disebut, misalnya "Apakah Produk X
  cocok untuk pajangan?" atau "Produk X layak untuk koleksi?", memakai
  product_detail dan mempertahankan Produk X sebagai satu-satunya objek.
  Jangan menggantinya dengan rekomendasi produk lain.
...
- Jika pertanyaan majemuk meminta bulk_discount sekaligus free_shipping tanpa
  meminta tarif ongkir tertentu, gunakan price_promo sebagai intent utama dan
  shipping_transaction sebagai intent tambahan.
...
- Permintaan harga produk, total harga, atau konversi total ke mata uang seperti
  USD tetap wajib mencantumkan goal price, meskipun nilainya baru dapat dihitung
  setelah produk atau ongkir dikonfirmasi.
...
- topic_relation membedakan new_topic, follow_up, topic_switch, dan
  ...
  topic_switch meskipun percakapan sebelumnya membahas produk lain. Kalimat
  seperti "abaikan yang tadi, sekarang cek Mazinger Z" wajib topic_switch.
