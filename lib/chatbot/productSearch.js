...
function scoreProduct(queryTokens, product = {}) {
  ...
  if (matchedTokens < minimumMatches) {
...
export function assessProductSearchConfidence(
  ...
  if (top._searchCoverage < 1 || top._discoveryScore < 4) {
    return {
      status: "not_found",
...
export function buildProductSearchClarification(decision = {}) {
  ...
  return (
    "Aku menemukan beberapa produk yang sama-sama cocok dengan nama tersebut:\n\n" +
    names.map((name) => `- **${name}**`).join("\n") +
    "\n\nSebutkan nama atau kode produk yang lebih lengkap agar aku tidak memilih produk yang salah."
  );
}
...
export function buildProductSearchOptions(decision = {}, intent = "") {
  ...
  return (decision.candidates || [])
    ...
    .slice(0, 3)
    .map((name) => ({ label: name, value: `${command} ${name}` }));
}
