...
function scoreProduct(queryTokens, product = {}) {
  ...
  const containsToken = (token, text) =>
    token.length === 1
      ? text.split(/\s+/).includes(token)
      : text.includes(token);
  ...
  for (const token of queryTokens) {
    if (containsToken(token, name)) {
      score += 5;
      matchedTokens += 1;
    } else if (fuzzyMatch(token, name)) {
      score += 4;
      matchedTokens += 1;
    } else if (containsToken(token, category)) {
      score += 3;
      matchedTokens += 1;
    } else if (fuzzyMatch(token, category)) {
      score += 2;
      matchedTokens += 1;
    } else if (containsToken(token, description)) {
      score += 1;
      matchedTokens += 1;
    }
  }
...
export function searchProductsForDiscovery(
  ...
  return products
    ...
    .sort((left, right) => {
      if (right._longestNameTokenRun !== left._longestNameTokenRun) {
        return right._longestNameTokenRun - left._longestNameTokenRun;
      }
