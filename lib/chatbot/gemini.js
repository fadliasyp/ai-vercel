...
function isFallbackableGeminiError(err) {
  const status = Number(getStatusCode(err));
  const msg = String(err?.message || err || "").toLowerCase();

  return (
    [400, 404, 408, 409, 429, 500, 502, 503, 504].includes(status) ||
    msg.includes("quota") ||
    msg.includes("rate") ||
...
export async function geminiGenerateContentWithFallback({
  ...
  const candidates = modelsFor(models || model);
  let lastError = null;
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (!isFallbackableGeminiError(err)) break;
    }
  }
...
export async function naturalizeWithGemini(payload, userQuestion) {
  try {
    if (!genai) return payload;
    if (!shouldUseGemini(userQuestion, payload)) return payload;
...
- Jangan pakai tabel.
- Jangan pakai JSON.
- Panjang 5-8 bullet/kalimat pendek, maksimal 900 karakter.
