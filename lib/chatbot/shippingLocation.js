export function normalizeLocationText(value = "") {
  return String(value || "")
    .toLowerCase()
    .replace(
      /^\s*(?:(?:iya+|ya+|betul|benar|maksud(?:ku|nya)?|itu|kota(?:nya)?|kabupaten(?:nya)?|kab(?:nya)?|kecamatan(?:nya)?|kec(?:nya)?|tujuan(?:nya)?)\s+)*/i,
      "",
    )
    .replace(/\s+(?:ya+|min|kak|mas|mbak)\s*$/i, "")
...
export function extractShippingDestination(question = "") {
  ...
  const routeMatch = /(?:ongkir(?:nya)?|ongkos\s+kirim|biaya\s+kirim|pengiriman|kirim)/i.test(
    text,
  )
    ? text.match(
        /\bdari\s+.+?\s+ke\s+(.+?)(?=\s+(?:ongkir(?:nya)?|untuk|produk|robot|barang|pakai|dengan|aman|asuransi|packing|kena|habis|sekitar|b(?:e+r+a+p+a+|r+p|r+a+p+a+)|dong|ya|kah|nih|min|kak)\b|[?.!]|$)/i,
      )
    : null;
  if (routeMatch?.[1]) return routeMatch[1].trim();

  const match = text.match(
    /(?:cek\s+)?(?:ongkir|ongkos\s+kirim|biaya\s+kirim|pengiriman|kirim)(?:\s+(?:ke|tujuan))?\s+(.+?)(?=\s+(?:dari|untuk|produk|robot|barang|pakai|dengan|aman|asuransi|packing|kena|habis|sekitar|b(?:e+r+a+p+a+|r+p|r+a+p+a+)|dong|ya|kah|nih|min|kak)\b|[?.!]|$)/i,
  );
...
export async function resolveShippingLocation(
  ...
  if (districtLookupFailed) {
    return { kind: "not_found", districtLookupUnavailable: true };
  }
