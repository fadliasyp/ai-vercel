...
export function extractShippingDestination(question = "") {
  ...
  const match = text.match(
    /(?:cek\s+)?(?:ongkir|ongkos\s+kirim|biaya\s+kirim|pengiriman|kirim)(?:\s+(?:ke|tujuan))?\s+(.+?)(?=\s+(?:untuk|produk|robot|barang|pakai|dengan|aman|asuransi|packing|berapa|dong|ya|kah|nih|min|kak)\b|[?.!]|$)/i,
  );
