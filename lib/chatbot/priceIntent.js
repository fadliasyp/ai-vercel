...
export function extractBudgetRange(question = "") {
  ...
  match = text.match(
    new RegExp(
      String.raw`(?:di atas|diatas|lebih dari|minimal|mulai dari)\s*${NUMBER}\s*${UNIT}\s*(?:dan\s*)?(?:di bawah|dibawah|kurang dari|maks(?:imal)?|max)\s*${NUMBER}\s*${UNIT}`,
    ),
  );
  if (match) {
  ...
  match = text.match(
    new RegExp(
      String.raw`(?:di bawah|dibawah|kurang dari|maks(?:imal)?|max)\s*${NUMBER}\s*${UNIT}\s*(?:dan\s*)?(?:di atas|diatas|lebih dari|minimal|mulai dari)\s*${NUMBER}\s*${UNIT}`,
    ),
  );
  if (match) {
    const sharedUnit = match[2] || match[4];
    return {
      detected: true,
      min: parseBudgetValue(match[3], match[4] || sharedUnit),
      max: parseBudgetValue(match[1], match[2] || sharedUnit),
    };
  }
...
export function extractRecommendationBudgetAnswer(question = "") {
  const range = extractBudgetRange(question);
  if (range.detected) return range;

  const text = normalizeBudgetText(question);
  const match = text.match(
    new RegExp(
      String.raw`^(?:rp\s*)?${NUMBER}\s*${UNIT}(?:\s*(?:aja|saja))?$`,
    ),
  );
  if (!match) return range;

  const value = parseBudgetValue(match[1], match[2]);
  const hasExplicitUnit = Boolean(match[2]) || /^rp\s*/.test(text);
  if (!value || (!hasExplicitUnit && value < 10000)) return range;

  return { detected: true, min: null, max: value };
}

export function isRecommendationBudgetFollowUp(
  lastBotQuestionType = "",
  question = "",
) {
  return (
    ["ask_budget", "ask_budget_value"].includes(
      String(lastBotQuestionType || ""),
    ) && extractRecommendationBudgetAnswer(question).detected
  );
}
