...
export function extractPromoSubjectKeywords(question = "") {
  const stopWords = new Set([
    "ada", "adakah", "aja", "apa", "apakah", "barang", "berapa", "buat",
    "cashback", "di", "diskon", "diskonnya", "disini", "dong", "engga", "gak",
    "ga", "hai", "halo", "hallo", "harga", "hari", "hello", "hi", "ini", "itu", "jadul", "kah", "kak", "kalian",
    "kalau", "kalo", "kami", "kamu", "koleksi", "kolektor", "lagi", "lihat", "min",
    "minggu", "ngga", "nggak", "nih", "penawaran", "potongan", "produk", "promo",
    "pcs", "promonya", "punya", "ready", "robot", "saat", "sale", "saja", "sekarang", "sedang", "sisa", "stock",
    "sih", "sini", "seluruh", "semua", "terbaru", "tersedia", "tidak", "toko",
    "tampilkan", "tokonya", "unit", "untuk", "website", "ya", "yang", "cari", "carikan",
  ]);
...
export function resolveProductQueryScope(question = "") {
  ...
  if (
    /\b(?:hasil|pilihan|daftar|produk(?:-produk|\s+produk)?|barang(?:-barang|\s+barang)?|robot(?:-robot|\s+robot)?)\s+(?:yang\s+)?(?:sebelumnya|tadi|di\s+atas)\b/.test(
      text,
    ) ||
    /\b(?:dari|berdasarkan)\s+(?:hasil|pilihan|daftar)\s+(?:sebelumnya|tadi|di\s+atas)\b/.test(
      text,
    ) ||
    /\b(?:dari|antara)\s+(?:(?:ke)?(?:dua|tiga|empat|lima)|\d+)\s+(?:(?:produk|barang|robot|pilihan)\s+)?(?:itu|tersebut|tadi)\b/.test(
      text,
    ) ||
    /\b(?:dari|antara)\s+(?:keduanya|ketiganya|keempatnya|kelimanya)\b/.test(
      text,
    )
  ) {
    return "previous";
  ...
  const asksForRecommendation =
    /\b(?:rekomendasi(?:kan)?|rekomen|cari|carikan|pilihkan|sarankan)\b/.test(
      text,
    );
  ...
  if (
    /\b(?:alternatif|opsi|pilihan)\s+(?:lain\s+)?(?:yang\s+)?(?:lebih\s+)?(?:worth\s+it|value\s+for\s+money|bagus|baik|murah|hemat)\b/.test(
      text,
    )
  ) {
    return "catalog";
