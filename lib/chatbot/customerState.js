...
export function detectCustomerState(question = "") {
  ...
  const postPurchaseIssue = looksLikePostPurchaseReturnIssue(text);
  const hypotheticalIssue =
    postPurchaseIssue &&
    /\b(?:kalau|jika|misalnya|seandainya)\b/.test(text);
  const returnProblem =
    looksLikeReturnPolicyQuestion(text) &&
    /\b(?:rusak|cacat|terkelupas|patah|retak|pecah|penyok|salah\s+kirim|tidak\s+sesuai|part\s+(?:kurang|hilang))\b/.test(
      text,
    );

  if (
    /\b(?:panik|kecewa|kesal|marah|emosi|kapok)\b/.test(text) ||
    ((postPurchaseIssue || returnProblem) && !hypotheticalIssue)
  ) {
