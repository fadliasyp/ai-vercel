...
function asksAlreadyVisibleProductFact(action = "", products = []) {
  ...
  return products.some((product) => {
    ...
    return (
      (asksPrice && hasVisiblePrice(product)) ||
      (asksStock && hasVisibleStock(product))
    );
  });
}
...
export function buildControlledActions(
  ...
  const withoutVisibleFacts = (actions) =>
    actions.filter(
      (action) => !asksAlreadyVisibleProductFact(action, payload.products),
    );
