...
const SUGGESTED_ACTION_RULES = [
  {
    ...
    pattern:
      /(?=.*\b(?:rekomendasi(?:kan)?|rekomen|pilihkan|sarankan)\b)(?=.*\b(?:budget|anggaran|dana|worth it|value for money)\b)/i,
  },
  {
...
function selectFreshActions(
  ...
  const ordered = eligible
    .map((action, index) => ({
      ...
      index,
      score:
        nextActionScore(userQuestion, action) -
        (recent.has(action.toLowerCase()) ? 100 : 0),
    }))
...
export function buildControlledActions(
  ...
  if (names.length === 1) {
    ...
    return selectFreshActions(actions, recentActions, limit, userQuestion);
  }
...
export function applyControlledFollowUpPolicy(
  payload,
  { intent = "general", recentActions = [], userQuestion = "" } = {},
) {
