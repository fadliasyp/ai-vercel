...
const GREETING_GLOBAL_ACTIONS = [
  "Tampilkan semua produk yang ready stock",
  "Lihat semua produk yang sedang promo",
...
const GREETING_SPECIFIC_ACTIONS = [
  "Cari produk kategori Chogokin",
  "Cari produk kategori Vintage",
  "Cari produk kategori Action Figure",
  "Cari produk kategori Transformers",
  "Cari produk kategori Super Sentai",
  "Cari produk kategori Kamen Rider",
  "Cari produk seri Getter Robo",
  "Cari produk seri Mazinger Z",
  "Cari produk seri Voltes V",
  "Cari produk seri Grendizer",
  "Cari produk seri Godmars",
  "Cari produk seri Ideon",
  "Cari produk Soul of Chogokin",
  "Cari produk Jumbo Machinder",
  "Cari produk Shokugan Modeling Project",
  "Cari produk Robot Damashii",
  "Cari produk DX robot",
  "Cari produk S.H.Figuarts",
];

const INTENT_ACTIONS = {
  greeting: [...GREETING_GLOBAL_ACTIONS, ...GREETING_SPECIFIC_ACTIONS],
  product_discovery: [
...
const SUGGESTED_ACTION_RULES = [
  {
    actionKey: "catalog_ready_stock",
    intent: "stock_availability",
    requiredFields: [],
    pattern:
      /^(?:cari robot yang ready stock|tampilkan semua produk yang ready stock|lihat daftar (?:robot|produk|barang) yang ready stock)$/i,
  },
  {
    actionKey: "catalog_promo",
    intent: "price_promo",
    requiredFields: [],
    pattern:
      /^(?:lihat semua produk yang sedang promo|lihat produk yang sedang promo|tampilkan semua produk promo)$/i,
  },
  {
...
export function validateSuggestedActionSelection(selection, question = "") {
  ...
  const productName = cleanProductName(selection.product_name);
  const hasProductReference =
    selection.product_id !== undefined ||
    selection.product_name !== undefined;
  if (!hasProductReference) return expected;
  ...
  if (
    ...
    productId <= 0 ||
    !productName ||
    !normalizeReference(expected.value).includes(
      normalizeReference(productName),
    )
  ) {
    return null;
  }
...
const NON_REPEATABLE_ACTION_FACETS = new Set([
  "shipping_quote",
  "insurance",
  "packing",
  "shipping_estimate",
...
function asksAlreadyAnsweredFacet(action = "", payload = {}) {
  const value = buildSuggestedActionMetadata(action)?.value || "";
  const facets = detectRequestedAnswerFacets(value).filter((facet) =>
    NON_REPEATABLE_ACTION_FACETS.has(facet),
  );
  if (!facets.length) return false;

  const coverage = evaluateAnswerCoverage(value, payload);
  return facets.every((facet) => coverage.status[facet] === "answered");
}

export function filterAnsweredSuggestedActions(actions = [], payload = {}) {
  return (Array.isArray(actions) ? actions : []).filter(
    (action) =>
      !asksAlreadyAnsweredProductFact(action, payload) &&
      !asksAlreadyAnsweredFacet(action, payload),
  );
}
...
function selectGreetingActions(recentActions = [], limit = 6) {
  const globalLimit = Math.min(4, limit);
  const specificLimit = Math.min(2, Math.max(0, limit - globalLimit));
  const orderedGlobals = selectFreshActions(
    shuffleActions(GREETING_GLOBAL_ACTIONS),
    recentActions,
    GREETING_GLOBAL_ACTIONS.length,
  );
  const diverseGlobals = [];
  const usedActionKeys = new Set();

  for (const action of orderedGlobals) {
    const actionKey = buildSuggestedActionMetadata(action)?.action_key;
    if (usedActionKeys.has(actionKey)) continue;
    usedActionKeys.add(actionKey);
    diverseGlobals.push(action);
    if (diverseGlobals.length === globalLimit) break;
  }

  if (diverseGlobals.length < globalLimit) {
    diverseGlobals.push(
      ...orderedGlobals
        .filter((action) => !diverseGlobals.includes(action))
        .slice(0, globalLimit - diverseGlobals.length),
    );
  }

  const specificActions = selectFreshActions(
    shuffleActions(GREETING_SPECIFIC_ACTIONS),
    recentActions,
    specificLimit,
  );
  return [...diverseGlobals, ...specificActions];
}
...
export function buildControlledActions(
  ...
  const withoutAnsweredFacts = (actions) =>
    filterAnsweredSuggestedActions(actions, payload);
  const contextualActions = CONTEXTUAL_ACTIONS[payload._actionContext];
  if (contextualActions) {
    return selectFreshActions(
      withoutAnsweredFacts(contextualActions),
      recentActions,
      limit,
      userQuestion,
    );
  ...
  if (normalizedIntent === "greeting") {
    return selectGreetingActions(recentActions, limit);
  }

  if (!PRODUCT_ACTION_INTENTS.has(normalizedIntent)) {
    return selectFreshActions(
      withoutAnsweredFacts(INTENT_ACTIONS[normalizedIntent]),
      recentActions,
      limit,
      userQuestion,
    );
  ...
  const withoutVisibleFacts = withoutAnsweredFacts;
