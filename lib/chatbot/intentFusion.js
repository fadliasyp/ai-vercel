...
const COMPARE_PATTERN =
  /\b(?:banding(?:kan)?|compare|versus|\bvs\b|beda(?:nya)?|perbedaan|sama\s+.+\s+beda)\b/i;
const RECOMMENDATION_PATTERN =
  /\b(?:rekomendasi|rekomen|mending|paling\s+cocok|bagusnya|worth\s+it|value\s+for\s+money|buat\s+(?:pajangan|hadiah|koleksi)|untuk\s+(?:pajangan|hadiah|pemula)|kolektor\s+baru)\b/i;
const PRICE_PATTERN =
  /\b(?:harga|sale|promo|diskon|cashback|budget|kisaran|di\s+bawah|dibawah|under|maks(?:imal)?\s+(?:budget|dana)|selisih\s+harga)\b/i;
const INSURANCE_PATTERN =
  /\b(?:asuransi|diasuransikan|jaminan\s+pengiriman)\b/i;
const TRACKING_PATTERN =
  /\b(?:resi|tracking|lacak|posisi\s+paket|paket\s+sampai\s+mana|perjalanan\s+kurir)\b/i;
const TRANSACTION_STATUS_PATTERN =
  /\b(?:status|progres|perkembangan)\s+(?:order|pesanan|pembayaran|transaksi)\b/i;
const STOCK_PATTERN =
  /\b(?:stok|stock|ready(?:\s+stock)?|tersedia|ketersediaan|restock|pre\s*order|preorder)\b/i;
