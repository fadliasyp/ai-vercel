...
import { looksLikeReturnPolicyQuestion } from "./storePolicy.js";
...
const PRODUCT_SUITABILITY_PATTERN =
  /\b(?:cocok|layak)\b[\s\S]*\b(?:pajangan|display|hadiah|kado|koleksi|kolektor|pemula)\b/i;
const PRICE_PATTERN =
...
const STOCK_PATTERN =
  /\b(?:stok|stock|ready(?:\s+stock)?|tersedia|ketersediaan|restock|pre\s*order|preorder|sold\s+out|sudah\s+habis)\b|\b(?:sisa|tersisa)\s*(?:berapa\s*)?(?:pcs|unit|buah)\b|\bberapa\s*(?:pcs|unit|buah)\b/i;
const SHIPPING_ORIGIN_PATTERN =
  /\b(?:dikirim|pengiriman)\s+dari\s+(?:kota\s+)?mana\b|\basal\s+kirim\b.*\bdari\s+mana\b|\bpaket\s+(?:nanti\s+)?berangkat\s+dari\s+mana\b|\b(?:alamat\s+(?:lengkap\s+)?toko|lokasi\s+(?:toko|offline|warehouse)|pickup\s+langsung|ambil\s+langsung|base\s+di)\b/i;
const SHIPPING_TRANSACTION_PATTERN =
  /\b(?:ongkir|ongkos\s+kirim|biaya\s+kirim|metode\s+pembayaran|bayar|pembayaran|pay\s*later|paylater|kredivo|akulaku|qris|transfer|cod|cash\s+on\s+delivery|checkout|cara\s+(?:beli|membeli|order|pesan)|kurir|ekspedisi|estimasi\s+(?:kirim|pengiriman|sampai|nyampe)|berapa\s+lama\s+(?:kirim|pengiriman)|packing|proteksi)\b/i;
const SHIPPING_QUOTE_PATTERN =
  /\b(?:ongkir|ongkos\s+kirim|biaya\s+kirim|cek\s+ongkir|tarif\s+pengiriman)\b/i;
const PRODUCT_DETAIL_PATTERN =
  /\b(?:detail|spesifikasi|spek|original|ori|ukuran|dimensi|berat|bahan|material|diecast|limited|edisi|versi|rilis|isi\s+box|kelengkapan|kondisi|misb|mib|bib|loose|junk|digerakkan|digerakin|artikulasi|kekurangan|kelebihan|minus|cacat|rusak|patah|retak|lecet|baret)\b/i;
const PRODUCT_DISCOVERY_COMMAND_PATTERN =
  /\b(?:cari|carikan|nyari|pencarian|daftar|list|pilihan|kategori)\b/i;
const PRODUCT_DETAIL_COMMAND_PATTERN =
  /\b(?:tampilkan|lihat|jelaskan)\s+(?:detail|spesifikasi|spek)\b/i;
...
export function looksLikeSingleProductSuitabilityQuestion(question = "") {
  const text = normalizeText(question);
  return (
    PRODUCT_SUITABILITY_PATTERN.test(text) &&
    !STRONG_RECOMMENDATION_PATTERN.test(text) &&
    !/\b(?:mana|carikan|pilihkan|alternatif|produk\s+lain|paling|ada\s+(?:yang|yg))\b/i.test(
      text,
    )
  );
}
...
export function detectExplicitIntentOverride(question = "") {
  ...
  if (looksLikeStoreBackgroundQuestion(text)) {
    return {
      intent: "general",
      method: "explicit_store_background_rule",
    };
  }
  ...
  if (looksLikeReturnPolicyQuestion(text)) {
  ...
  if (
    looksLikeShippingOriginQuestion(text) ||
    SHIPPING_ORIGIN_PATTERN.test(text)
  ) {
    return {
  ...
  if (isCODQuestion(text)) {
    return {
      intent: "shipping_transaction",
      method: "explicit_cod_rule",
    };
  }
  if (looksLikePaymentMethodQuestion(text)) {
    return {
      intent: "shipping_transaction",
      method: "explicit_payment_methods_rule",
    };
  }
  ...
  if (looksLikePackingProtectionQuestion(text)) {
    return {
      intent: "shipping_transaction",
      method: "explicit_shipping_packing_rule",
    };
  }
  if (looksLikeShippingEstimateQuestion(text)) {
    return {
      intent: "shipping_transaction",
      method: "explicit_shipping_estimate_rule",
    };
  }
  if (looksLikeShippingCoverageQuestion(text)) {
    return {
      intent: "shipping_transaction",
      method: "explicit_shipping_coverage_rule",
    };
  }
  if (looksLikeHowToBuyQuestion(text)) {
    return {
      intent: "shipping_transaction",
      method: "explicit_how_to_buy_rule",
    };
  }
  if (SHIPPING_QUOTE_PATTERN.test(text)) {
    return {
      intent: "shipping_transaction",
      method: "explicit_shipping_quote_rule",
    };
  }
  if (looksLikeSingleProductSuitabilityQuestion(text)) {
    return {
      intent: "product_detail",
      method: "explicit_product_suitability_rule",
    };
  }
  if (
  ...
  if (PRODUCT_DETAIL_COMMAND_PATTERN.test(text)) {
    return { intent: "product_detail", method: "explicit_product_detail_rule" };
  }
  if (
    PRODUCT_DETAIL_PATTERN.test(text) &&
    !PRODUCT_DISCOVERY_COMMAND_PATTERN.test(text)
  ) {
    return { intent: "product_detail", method: "explicit_product_detail_rule" };
  }
...
export function chooseSemanticIntent({
  ...
  const semanticProvider = String(semantic?.provider || "groq")
    .trim()
    .toLowerCase();
  ...
  if (localScope === "out_of_scope" && !llmLed) {
    return {
      ...localResult,
      intent: "general",
  ...
  if (!semantic) {
    return {
      ...localResult,
      ...(explicit || {}),
      ...(explicit ? { score: 1, scope: "in_scope" } : {}),
    };
  }
  ...
  if (semanticScope === "out_of_scope") {
    return {
      ...localResult,
      intent: "general",
      method: `${semanticProvider}_out_of_scope`,
